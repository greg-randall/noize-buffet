...
**Time and usage.** Measured on one run: chat replies took about 8 seconds, and a first batch with web research took about 4 minutes. The agent runs on whatever account Claude Code is logged in with. On a Claude subscription there's no per-job charge, but jobs count toward your plan's usage limits, shared with your own Claude Code use; that researched batch used about as much as 20 chat replies. With an API key instead, you pay per job. Either way, the worker terminal prints each job's API-equivalent cost as a measure of how heavy it was (about $0.05 per chat reply and $1 for that batch). The only other paid service is TypeSafe, for comment mining once that exists: about $0.02 per 1,000 comments.
